...
function StatCard({ title, value, subtitle, icon: Icon, iconTone }) {
  return (
    <article className="min-h-[104px] rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
      ...
      <div className="mt-3">
        ...
        <p className="mt-1 text-[28px] font-bold leading-none text-slate-950">{value}</p>
      </div>
    </article>
  );
}
...
function TaskCard({ task }) {
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-3 text-left shadow-[0_8px_22px_rgba(15,23,42,0.04)] transition hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-[0_14px_30px_rgba(15,23,42,0.08)]">
      <div className="flex items-start justify-between gap-3">
        <div>
        ...
        {task.status === 'done' ? <CheckCircle2 className="h-4 w-4 flex-none text-emerald-500" /> : null}
      </div>
      ...
      <div className="mt-3 rounded-xl bg-slate-50 p-2.5">
        ...
        <p className="mt-1 text-[11px] font-semibold text-slate-400">{task.checklist}</p>
      </div>

      <p className="mt-3 text-xs font-medium leading-5 text-slate-500">{task.note}</p>
    </article>
  );
}
...
function BoardColumn({ column, tasks }) {
  return (
    <section className="flex min-h-[520px] w-[252px] flex-none flex-col rounded-2xl border border-slate-200 bg-white/75 p-3 shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
      <div className="flex items-center gap-2">
        <span className={`h-2.5 w-2.5 rounded-full ${column.dot}`} />
        <h2 className="min-w-0 flex-1 truncate text-sm font-bold text-slate-900">{column.title}</h2>
...
export default function MyTasksPage() {
  ...
  return (
    ...
    >
      <div className="grid gap-5 px-5 py-5 xl:grid-cols-[1fr_290px] xl:px-7">
        <section className="space-y-5">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_16px_40px_rgba(39,48,93,0.08)]">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-bold text-slate-900">Meine Aufgaben Uebersicht</p>
                <p className="mt-1 text-sm text-slate-500">
                  Alle dir zugewiesenen Aufgaben auf einen Blick, inklusive Stand, Deadline und offenen Baustellen.
                </p>
              </div>
              <div className="rounded-full bg-violet-50 px-3 py-2 text-xs font-bold text-[#6047e8]">
        ...
        <aside className="space-y-5">
            ...
            <div className="mt-4 space-y-3">
              {focusTasks.map((task) => (
                <div key={task.id} className="rounded-xl bg-slate-50 p-3">
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-400">{task.project}</p>
                  <p className="mt-2 text-xs font-medium text-slate-500">{task.note}</p>
                </div>
              ))}
            </div>
          </SideCard>
          ...
          <SideCard title="Naechster Schritt">
            <div className="mt-4 rounded-2xl bg-violet-50 p-4">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#6047e8]">Empfohlen</p>
            ...
            <div className="mt-4 space-y-3">
              {activityFeed.map((item) => (
                <div key={item} className="flex items-start gap-3 rounded-xl bg-slate-50 p-3">
                  <span className="mt-0.5 inline-flex h-8 w-8 flex-none items-center justify-center rounded-full bg-white text-[#6047e8]">
                    <MessageSquareMore className="h-4 w-4" />
                  </span>
                  <p className="text-sm font-medium leading-5 text-slate-600">{item}</p>
                </div>
              ))}
            </div>
          </SideCard>

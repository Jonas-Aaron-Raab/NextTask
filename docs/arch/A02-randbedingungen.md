...
| TECH-02 | **Node.js als Laufzeit, npm als Paketmanager** | Drei `package.json`: Wurzel (`concurrently` zum gemeinsamen Start, `@playwright/test` für Rauchtest und Schnittstellentests), `server/`, `client/`. Keine Mindestversion festgelegt (kein `engines`-Feld); Entwicklung mit aktueller LTS. |

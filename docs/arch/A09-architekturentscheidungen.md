...
**Konsequenzen.** `html2canvas` und `jspdf` als Abhängigkeiten der Browser-Anwendung. Bericht immer dreiseitig, auch bei leeren Listen (B3). Fünf Erläuterungszeilen kommen aus dem Ampelwert statt aus den Notizfeldern (B3, Hinweis); das ist eine Lücke der Vorlage, nicht der Entscheidung. Wird die Berichtsbasis an den Server angebunden, kann die Erzeugung im Browser bleiben; nur die Datenquelle wechselt. Ausführlich: [`adr/005-pdf-im-browser.md`](../../adr/005-pdf-im-browser.md).

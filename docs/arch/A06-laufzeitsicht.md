...
- **Sichtbereich vor Zustand.** Die Anfrage wird mit `findUnique` geladen; danach prüft `userCanAccessEntity` ihr Bezugsobjekt gegen den Sichtbereich (`findFirst` auf Projekt, Aufgabe, Statusbericht oder Dokument mit dem passenden `build…ScopeWhere` aus `accessScope.js`; freie Anliegen ohne Bezugsobjekt gelten immer als sichtbar). Liegt das Bezugsobjekt außerhalb, antwortet der Server 404, bevor Zustand oder Berechtigung geprüft werden. Danach folgt: Zustand, dann Vier-Augen-Prüfung (`before.requesterId === currentUser.id` → 403, seit 24. September), dann Berechtigung. Ein berechtigter Anwender erfährt so, dass die Anfrage existiert und entschieden ist; für ein internes Werkzeug ist das gewollt, damit die Meldung im Cockpit stimmt.

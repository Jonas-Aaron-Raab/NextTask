...
| SC-02 | Jede Freigabeanfrage durchläuft genau einen Weg von `PENDING` nach `APPROVED`, `REJECTED` oder `CANCELLED`. Der Anfragende wird nie als Genehmiger eingetragen und kann seine Anfrage nicht selbst entscheiden. Zu jeder Entscheidung sind Entscheider, Zeitpunkt und Vermerk gespeichert. |
...
| R-04 | **Löschen einer Rolle mit zugeordneten Benutzern.** Bis zum 24. September 2026 erhielten die Benutzer einer gelöschten Rolle die Systemrolle Admin als Ersatz. Seitdem lehnt der Server das Löschen ab, solange Benutzer zugeordnet sind (UC-21). | Behoben. Das Löschen bleibt nur mit „Rollen verwalten" möglich und wird mit Kritikalität `CRITICAL` protokolliert. |
